...
<body>
	...
	%>
		...
		<div id="divnotice">
			...
			//notice.jsp화면 요청시...아래쪽의 페이지번호를 클릭 하지 않은 상태이다...
			...
			//board테이블에서 검색한 글의 정보를 저장할 용도의 List 인터페이스 타입의 변수 list선언
			List<BoardBean> list = null; 
			
			//만약 board테이브에 글이 존재 한다면
			if(count > 0){
		...
		<!-- 게시판 -->
			...
			<table id="notice">
				...
				<%
					if(count > 0){//게시판의 글이 존재 하면
						
						for(int i=0;i<list.size();i++){//ArrayList사이즈 만큼 반복 (BoardBean 객체 개수 만큼)
				...
				%>
						<tr onclick="location.href='content.jsp?num=<%=bean.getNum() %>&pageNum=<%=pageNum %>'">
							...
							<td class="twrite"><%=bean.getName() %></td>
							<td><%=new SimpleDateFormat("yyyy.MM.dd").format(bean.getDate()) %></td>
							<td class="ttread"><%=bean.getReadcount() %></td>
						</tr>
				<%
						}
